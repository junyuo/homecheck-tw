...
test('市場驗收要求雙北各五筆、所有欄位與各市門牌索引證據一致', () => {
  ...
  const samples = ['taipei', 'new-taipei'].flatMap((city) =>
    Array.from({ length: 5 }, (_, index) => ({
      ...
      result: 'matched',
      verificationMethod: 'official-raw-offline',
      marketOwnership: city === 'taipei' && index >= 3 ? 'private' : 'public',
      classificationMethod: city === 'taipei' && index >= 3
        ? 'official-private-registry'
        : city === 'taipei'
          ? 'fresh-stall-count'
          : 'official-type-traditional-market',
      fields: {
        name: true,
        id: true,
        district: true,
  ...
  const audit = { status: 'passed', adapterVersion: 'market-v2', samples }
